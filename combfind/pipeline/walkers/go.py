class GoWalker:
    def extract_symbols(self, root, module_name: str) -> list[dict]:
        ...
        pkg_node = next((c for c in root.named_children if c.type == "package_clause"), None)
        if pkg_node:
            pkg_id = next((c for c in pkg_node.named_children if c.type == "package_identifier"), None)
            if pkg_id:
        ...
        for node in root.named_children:
            if node.type == "function_declaration":
                ...
                params = next((c for c in node.named_children if c.type == "parameter_list"), None)
                ...
                name_node = next((c for c in node.named_children if c.type == "field_identifier"), None)
                if name_node is None:
                    continue
                name = name_node.text.decode("utf-8")
                param_lists = [c for c in node.named_children if c.type == "parameter_list"]
                params = param_lists[1] if len(param_lists) > 1 else None
                recv_str = param_lists[0].text.decode("utf-8") if param_lists else ""
                sig = f"func {recv_str} {name}{params.text.decode('utf-8') if params else '()'}"
                qualified = f"{package}.{recv_type}.{name}" if recv_type else f"{package}.{name}"
                results.append({
                    "name": name,
                ...
                for spec in node.named_children:
                    if spec.type != "type_spec":
                        continue
                    name_node = next((c for c in spec.named_children if c.type == "type_identifier"), None)
                    if name_node is None:
                        continue
                    name = name_node.text.decode("utf-8")
                    body = spec.named_children[-1] if spec.named_children else None
                    kind = "interface" if (body and body.type == "interface_type") else "struct"
                    results.append({
                        "name": name,
                        "qualified_name": f"{package}.{name}",
                        "kind": kind,
    ...
    for param in recv.named_children:
        for child in param.named_children:
            ...
            if child.type == "pointer_type":
                inner = next((c for c in child.named_children if c.type == "type_identifier"), None)
                if inner:
