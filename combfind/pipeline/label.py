    ...
    **_,
    ...
    if backend is None:
        if llm_model is None:
            raise ValueError(
                "llm backend or --llm-model is required for the label stage"
            )
    ...
    unlabeled = conn.execute("SELECT id FROM concepts WHERE name IS NULL").fetchall()
    ...
    with ThreadPoolExecutor(max_workers=llm_workers) as ex:
        ...
        for future in as_completed(futures):
            ...
            telemetry.debug(
                "label concept", progress=f"{completed}/{total}", concept_id=concept_id
            )
            ...
            telemetry.debug(
                "concept labeled", progress=f"{completed}/{total}", name=name, role=role
            )
            conn.execute(
...
def _build_messages(members) -> list[dict]:
    lines = []
    for m in members:
        qname = m["qualified_name"] or m["name"]
        sig = m["signature"] or m["name"]
        line = f"- {qname} ({m['kind']}): {sig}"
        if m["docstring"]:
    ...
    return [
        {
            ...
            "content": (
                ...
                "Given a cluster of related code symbols, "
                "output a JSON object with exactly these fields:\n"
                '  "name": a short concept name (2-5 words) — must be specific, '
                'never generic terms like "function", "class", "code", '
                'or "interface"\n'
                '  "description": one sentence a developer would search for to '
                "find this code — "
                "mention specific technologies, patterns, file types, or use cases; "
                "write it so queries like 'how do I add Go support?' or "
                "'where is auth handled?' would match\n"
