...
@click.option(
    ...
    show_envvar=True,
    help=f"Model: GGUF path (local), HF repo (mlx). "
    f"Default: ~/.cache/combfind/models/{_DEFAULT_FILE}",
)
@click.option(
    "--llm-mode",
    default="local",
...
def init_cmd(
    ...
    resolved_llm = model or _default_model_path()

    if llm_mode == "local" and resolved_llm is None:
        raise click.ClickException(
            f"no model found; run `combfind download-model` to fetch the default "
            f"({_DEFAULT_FILE}), or pass --model / set COMBFIND_MODEL"
        )
    if llm_mode == "mlx" and resolved_llm is None:
        raise click.ClickException(
            "--model is required for mlx mode (HuggingFace repo ID or local path)"
        )
...
@click.option(
    ...
    envvar="COMBFIND_MODEL",
    show_envvar=True,
    help=f"Model for --rerank/--agentic: GGUF path (local), HF repo (mlx). "
    f"Default: ~/.cache/combfind/models/{_DEFAULT_FILE}",
)
def query_cmd(text, db, top_k, fmt, rerank, agentic, agentic_limit, llm_mode, model):
    ...
    if rerank or agentic:
        ...
        backend = create_backend(llm_mode, llm_model=model or _default_model_path())
