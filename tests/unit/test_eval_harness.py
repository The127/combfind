...
def test_perfect_symbol_recall(tmp_path, mock_query_hit):
    fixtures = _fixture_dir(
        tmp_path,
        {
            "case1": (
                "find auth code",
                {"files": [], "symbols": ["AuthService.validate"]},
            )
        },
    )
    agg = harness_mod.run(db_path="fake.db", fixtures_dir=fixtures, ks=[1])
...
def test_zero_recall_on_miss(tmp_path, mock_query_miss):
    fixtures = _fixture_dir(
        tmp_path,
        {
            "case1": (
                "find auth code",
                {"files": ["auth/service.py"], "symbols": ["AuthService.validate"]},
            )
        },
    )
    agg = harness_mod.run(db_path="fake.db", fixtures_dir=fixtures, ks=[1])
...
def test_empty_fixtures_dir(tmp_path):
    ...
    agg = harness_mod.run(
        db_path="fake.db", fixtures_dir=str(tmp_path / "fixtures"), ks=[3]
    )
    assert agg == {}
...
def test_skips_incomplete_fixture(tmp_path, mock_query_hit):
    ...
    (valid / "expected.json").write_text(
        json.dumps({"files": ["auth/service.py"], "symbols": []})
    )
